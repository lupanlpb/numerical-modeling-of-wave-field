...
!----------------生成波场快照文件名结束-------------------------
  ...
  Do i=1,n_file2
    Open(i+300,file=seismogram_x(i),status='replace')
    Open(i+400,file=seismogram_z(i),status='replace')
  End Do
  ...
  Do it=1,nt2
    If(mod(it,2).ne.0) Then   !若是奇数则进行下列操作
      ...
      nx1=nx+ml+1
      Do i=ml,5,-1      !i=1--->-(ml-1), i=ml--->nx+1	  右边界
        Do j=1,nz
    ...
    Else
      ...
      Do i=1,nx
        Do j=1,nz
          ...
          txx(i,j,2)=txx(i,j,1)+r1*Lamda_2Miu(i,j)*(ef1*a11-ef2*a12+ef3*a13-ef4*a14)+r2*Lamda(i,j)*(ef1*a21-ef2*a22+ef3*a23-ef4*a24)    
          tzz(i,j,2)=tzz(i,j,1)+r2*Lamda_2Miu(i,j)*(ef1*b11-ef2*b12+ef3*b13-ef4*b14)+r1*Lamda(i,j)*(ef1*b21-ef2*b22+ef3*b23-ef4*b24)    
